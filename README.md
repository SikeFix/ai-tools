...
- **AI 编程 · Viable Coding 板块**：教你把 AI 写的代码打磨成真正可用的产品。聚合页含「什么是 Viable Coding」概念区、4 阶段学习路径（Vibe 入门 → Context Engineering → Agentic Engineering → 打磨交付）与 20 个精选 GitHub 学习资源（5 组：入门 / Context Engineering / Agentic Engineering / 打磨实战 / 资源清单），**每个资源有独立站内教学页**（简介 / 适合人群 / 核心内容章节 / 实践建议 / 配套工作流 / 同类资源，双语），内容基于各仓库 README 原创整理、标注来源，并配套教学工作流直达。
...
编辑 `src/data/viable-coding.json` 的 `resources` 数组加一条：`slug / name / url / stars / category / tags / descZh / descEn`（`category` 需在 `categories` 数组内，`slug` 需唯一）；同时需在 `src/data/viable-teaching/` 新增同名教学文件（`introZh/introEn` + `audienceZh/audienceEn` + `sectionsZh/sectionsEn` + `practiceZh/practiceEn`，结构见 `claude-code-best-practice.json`），`npm run validate` 会强制检查教学页覆盖。配套教学 SOP 见「如何新增 SOP」。
