...
- **SOP 板块**：19 套可复用的 AI 工作流，按「视频制作 / 内容创作 / 效率提升」分组（视频：口播/文生视频/数字人/电影级/数字人直播/剪辑切片；内容：写作/出图/翻译/播客/社媒/音乐；效率：编程/PPT/数据分析/会议/学习/客服/电商），含分步骤、工具、耗时与避坑清单；工作流内工具可直达站内工具详情页。
- **Skills 板块**：58 个常用 Claude Code / AI 技能与 MCP 目录（含 superpowers / ECC / karpathy-skills 等热榜），每个技能有独立详情页（能做什么 / 安装方式 / 来源 / 相关技能），标注官方/社区/内置来源。
- **全局体验**：工具 / SOP / Skills 三页均有搜索过滤；工具详情「推荐工作流」直达相关 SOP，SOP 内工具直达工具详情（双向互链）；SOP 详情含 HowTo 结构化数据；回到顶部按钮；增强页脚导航；键盘 focus 可访问性。
...
在 `src/data/sop/zh/` 与 `src/data/sop/en/` 各加一份同名 JSON（结构见现有 4 份），`validate` 会检查 zh/en 一一对应。SOP 卡片按 slug 排序（plan-a → plan-d）。
